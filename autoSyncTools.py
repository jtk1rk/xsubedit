...
def gen_match_array(slc, spec, doffset, break_limit):
    ...
    dropped = 0
    for i in range(spec[2].shape[1] - slc.shape[1] - offset):
...
def get_mean_sum_value(slc, dst):
    ridx = [ get_random_in_range(0, dst[2].shape[1] - slc.shape[1]) for i in range(3) ]
    res = []
    for offset in ridx:
...
def get_spec_slice(start_msec, stop_msec, spec):
    start_sec = start_msec / 1000
    stop_sec = stop_msec / 1000
    td = spec[1][1] - spec[1][0]
    low = int(floor(start_sec / td))
    high = int(ceil(stop_sec / td))
...
def match(src_spec, dst_spec, _start_ms, _stop_ms, offset = 0, break_limit = -1):
    ...
    _break_limit = -1 if break_limit == -1 else round(int(break_limit / 28))
